function ThisSheet() {
  ...
  **************/
  ...
  if (encontrado) {
    ...
    while (folders.hasNext()) {
      ...
      else if (/^Test Check in/.test(folder.getName())) this._directorio_checkin = folder;
      /**************
         Feedback
      **************/
  ...
  if (!this._directorio_checkout)
    this._directorio_checkout = this._directorio.createFolder('Test Check out');
     ...
     if (!this._directorio_checkin)
    this._directorio_checkin = this._directorio.createFolder('Test Check in');
  ...
  **************/
  ...
  this.addTestCheckin = function(formulario) {
    this._addFormulario(formulario, this._directorio_checkin);
  ...
  this.getFormularioTestFeedback = function() {
    ...
    while (files.hasNext()) {
      ...
      if (
        ...
        (!excel_responses && /Cuestionario Alumnos Data Analyst$/.test(file.getName()))
        ) {
          form = file;
        }
    }
    ...
    this.getRespuestasTestCheckin= function ()  {    return this._getCheckin(true);  }
  this.getFormularioTestCheckin= function ()  {    return this._getCheckin();  }


  this._getCheckin= function (excel_responses)
  {
      var form  = null,
          folder= this._directorio_checkin;

      var files = folder.getFiles();
       while (files.hasNext())
       {
         var file = files.next();
         if((excel_responses && /Check-in \(respuestas\)$/.test(file.getName())) ||
            (!excel_responses && /Check-in$/.test(file.getName())))
         {
           form=file;
         }
       }
