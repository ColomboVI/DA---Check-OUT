...
function CheckinTest()
{
  ...
  DriveApp.getFileById(this._formulario.getId()).setName('Check-in');
  DriveApp.getFileById(this._excel.getId()).setName('Check-in (respuestas)');
    ...
    this.getPreguntasTestCheckOut=function()
{
    if (this._array_preguntas_checkout)
      ...
      if (this._pais && this._pais.getAbreviatura()=="TK")
...
this.getPreguntasTestCheckIn=function()
    {
      ...
      //(Alex)Comentado para prueba
      //if (this._pais.getAbreviatura()=="TK")
       //   programa_pais = programa_pais + "-" + this._pais.getAbreviatura();

        //var sheet_test=SpreadsheetApp.openById('1k-VQbnIAgqU9Usy45RZiSiQCnljNeKDye7wvF3txmGg');
      var sheet_test=SpreadsheetApp.openById('1yLKUDqkr4ImuH07MWK34y7ElNztoQoQE1-ZxE2mBw9I');
